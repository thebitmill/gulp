...
function sassErrorHandler (err) {
  if (err.plugin === 'gulp-sass') {
    ...
    if (/\sstdin\s/.test(err.message)) { err.message = err.message.replace(/\sstdin\s/, path) }
  }
...
gulp.task('sass', () => {
  if (sassConfig.suffix) { fs.writeFile(sassConfig.dest + '.json', JSON.stringify({ suffix: sassConfig.suffix })) }
  ...
  if (sassConfig.suffix) { pipe = pipe.pipe(rename({ suffix: sassConfig.suffix })) }
